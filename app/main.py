...
def create_initial_users():
    ...
    users = [["user1", "user1@yandex.ru", "password1"],["user2", "user2@yandex.ru", "password2"],["user3", "user3@yandex.ru", "password3"]]
    for i in range(len(users)):
        user = User(username=users[i][0], email=users[i][1], password=users[i][2])
        db.add(user)
    db.commit()
    db.close()
